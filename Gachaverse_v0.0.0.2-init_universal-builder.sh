...
apt update -qq
apt upgrade -y -qq
apt install -y -qq build-essential scons pkg-config libx11-dev libxcursor-dev libxinerama-dev libgl1-mesa-dev libglu1-mesa-dev libasound2-dev libpulse-dev libdbus-1-dev libudev-dev libxi-dev libxrandr-dev unzip wget openjdk-17-jdk python3 python3-pip git optipng
...
sdk/cmdline-tools/latest/bin/sdkmanager --sdk_root=sdk "platform-tools" "platforms;android-35" "build-tools;35.0.0" "ndk;23.2.8568313"
...
mv temp_templates/templates/* templates/4.4.stable/
...
optipng -o7 -strip all icons/icon.png
gdown -q "1LzVS0NIhZmeeTdxcTLWD-1DrJVaHRnep" -O app/assets/background.png
optipng -o7 -strip all app/assets/background.png
gdown -q "1RPyR4l8lePcyTGVgbtOGCDM_HkAq2dK8" -O app/assets/music.mp3
gdown -q "1J5SQDrN1pPIRPHIiDnHPjocbU47T8UPb" -O app/assets/main_background.png
optipng -o7 -strip all app/assets/main_background.png
gdown -q "17S6EpsSoBeKRpO-HA8kRN6kdeZyTjU6S" -O app/assets/main_music.mp3
...
optipng -o7 -strip all app/icon.png
...
[ext_resource type="Texture2D" path="res://assets/main_background.png" id="1"]
[ext_resource type="AudioStream" path="res://assets/main_music.mp3" id="2"]
...
autoplay = true
EOF
keytool -genkeypair -keystore key/release.keystore -alias gachaverse -keyalg RSA -keysize 2048 -validity 10000 -storepass gachaverse -keypass gachaverse -dname "CN=Gachaverse,OU=Void,O=ICHxTenebra,L=Unknown,ST=Unknown,C=XX"
...
cd app
GODOT_SILENCE_ROOT_WARNING=1 JAVA_HOME=/usr/lib/jvm/java-17-openjdk-amd64 ANDROID_HOME="$(dirname $(pwd))/sdk" ../godot --headless --export-release "Android" "../out/Gachaverse_v0.0.0.2-init_universal.apk" &
PID=$!
while true; do
    sleep 2
    [ -f "../out/Gachaverse_v0.0.0.2-init_universal.apk" ] && [ $(stat -c%s "../out/Gachaverse_v0.0.0.2-init_universal.apk" || echo 0) -gt 1000000 ] && break
done
kill $PID
cd ..
cat > app/export_presets.cfg << EOF
...
echo "4.4.stable" > app/android/.build_version
cd app
GODOT_SILENCE_ROOT_WARNING=1 JAVA_HOME=/usr/lib/jvm/java-17-openjdk-amd64 ANDROID_HOME="$(dirname $(pwd))/sdk" ../godot --headless --export-release "Android" "../out/Gachaverse_v0.0.0.2-init_arm64-v8a.apk" &
PID=$!
while true; do
    sleep 2
    [ -f "../out/Gachaverse_v0.0.0.2-init_arm64-v8a.apk" ] && [ $(stat -c%s "../out/Gachaverse_v0.0.0.2-init_arm64-v8a.apk" || echo 0) -gt 1000000 ] && break
done
kill $PID
cd ..
cat > app/export_presets.cfg << EOF
...
echo "4.4.stable" > app/android/.build_version
cd app
GODOT_SILENCE_ROOT_WARNING=1 JAVA_HOME=/usr/lib/jvm/java-17-openjdk-amd64 ANDROID_HOME="$(dirname $(pwd))/sdk" ../godot --headless --export-release "Android" "../out/Gachaverse_v0.0.0.2-init_armeabi-v7a.apk" &
PID=$!
while true; do
    sleep 2
    [ -f "../out/Gachaverse_v0.0.0.2-init_armeabi-v7a.apk" ] && [ $(stat -c%s "../out/Gachaverse_v0.0.0.2-init_armeabi-v7a.apk" || echo 0) -gt 1000000 ] && break
done
kill $PID
cd ..
cat > app/export_presets.cfg << EOF
...
echo "4.4.stable" > app/android/.build_version
cd app
GODOT_SILENCE_ROOT_WARNING=1 JAVA_HOME=/usr/lib/jvm/java-17-openjdk-amd64 ANDROID_HOME="$(dirname $(pwd))/sdk" ../godot --headless --export-release "Android" "../out/Gachaverse_v0.0.0.2-init_x86_64.apk" &
PID=$!
while true; do
    sleep 2
    [ -f "../out/Gachaverse_v0.0.0.2-init_x86_64.apk" ] && [ $(stat -c%s "../out/Gachaverse_v0.0.0.2-init_x86_64.apk" || echo 0) -gt 1000000 ] && break
done
kill $PID
cd ..
cat > app/export_presets.cfg << EOF
...
echo "4.4.stable" > app/android/.build_version
cd app
GODOT_SILENCE_ROOT_WARNING=1 JAVA_HOME=/usr/lib/jvm/java-17-openjdk-amd64 ANDROID_HOME="$(dirname $(pwd))/sdk" ../godot --headless --export-release "Android" "../out/Gachaverse_v0.0.0.2-init_x86.apk" &
PID=$!
while true; do
    sleep 2
    [ -f "../out/Gachaverse_v0.0.0.2-init_x86.apk" ] && [ $(stat -c%s "../out/Gachaverse_v0.0.0.2-init_x86.apk" || echo 0) -gt 1000000 ] && break
done
kill $PID
cd ..
echo ""
